...
module.exports = {
    ...
    uploadPhoto: function (req, res) {
        if (!req.file)
            return res.json({status: false, error: 'You must provide a photo.'});
        // Add to database
        db.query('INSERT INTO `users_uploads` SET `user_id` = ?, `name` = ?, `created_at` = ?', [
           ...
           if (err) {
            ...
            res.json({status: true, success: 'Your image has been saved!', data: {
                upload: {
                    id: result.insertId,
    ...
    updateBio: function (req, res) {
        ...
        validator(accountModel, req.body, function (err, status) {
            ...
            if (!req.body.tags.match(/^([A-Za-z0-9 \-_]+,?)+$/))
                return res.json({status: false, error: 'Your tags are invalid.'});
                ...
                accountModel.computePopularity(user, function (err, popularity) {
                    ...
                    if (req.session.user !== user.id) {
                        ...
                        userModel.get(req.session.user, function (err, userFound) {
                            if (userFound)
                                notification.send(user.id, '<a href="/' + userModel.htmlentities(userFound.username) +'">' + userModel.htmlentities(userFound.username) + '</a> just visited your profile !', io, userFound.id);
                        });
        ...
        db.query('SELECT `users_accounts`.`sexual_orientation`, `users_accounts`.`gender`, `users_accounts`.`location`, `users_accounts`.`age`, `users_accounts`.`tags` ' +
            ...
            if (req.body.age) {
                ...
                if (age.length !== 2 || parseInt(age[0]) != age[0] || parseInt(age[1]) != age[1])
                    return res.json({status: false, error: 'Age is invalid.'});
            ...
            if (req.body.popularity) {
                ...
                if (popularity.length !== 2 || parseInt(popularity[0]) != popularity[0] || parseInt(popularity[1]) != popularity[1])
                    return res.json({status: false, error: 'Popularity invalidates.'});
                if (popularity[0] > 0) {
            ...
            db.query(sql, values, function (err, rows) {
                if (err) {
                    console.error(err);
                    return res.json({status: false, error: 'An error has occurred'});
                }
                ...
                async.eachOf(rows, function (match, key, next) {
                    ...
                    return res.json({status: true, success: rows.length + ' users found.', data: rows});
                });
            });
        });
    },
            ...
            db.query('DELETE FROM `likes` WHERE `user_id` = ? AND `liked_id` = ?', [req.session.user, user[0].id], function (err, rows) {
                ...
                if (rows.affectedRows > 0) {
                    db.query('SELECT `id` FROM `likes` WHERE `user_id` = ? AND `liked_id` = ?', [user[0].id, req.session.user], function (err, data) {
                        ...
                        userModel.get(req.session.user, function (err, userFound) {
                            if (userFound)
                                notification.send(user[0].id, '<a href="/' + userModel.htmlentities(userFound.username) +'">' + userModel.htmlentities(userFound.username) + '</a> don\'t like your profile anymore!', io, userFound.id);
                        });
                    });
                    return res.send();
                }
                ...
                db.query('INSERT INTO `likes` SET `user_id` = ?, `liked_id` = ?', [req.session.user, user[0].id], function (err) {
                    ...
                    db.query('SELECT `id` FROM `likes` WHERE `user_id` = ? AND `liked_id` = ?', [user[0].id, req.session.user], function (err, data) {
                        ...
                        if (!data || !data.length) {
                            userModel.get(req.session.user, function (err, userFound) {
                                if (userFound)
                                    notification.send(user[0].id, '<a href="/' + userModel.htmlentities(userFound.username) +'">' + userModel.htmlentities(userFound.username) + '</a> just liked your profile!', io, userFound.id);
                            });
                            return;
                        }
                        userModel.get(req.session.user, function (err, userFound) {
                            if (userFound)
                                notification.send(user[0].id, '<a href="/' + userModel.htmlentities(userFound.username) +'">' + userModel.htmlentities(userFound.username) + '</a> just liked your profile too!', io, userFound.id);
                        });
                    });
                    res.send();
                })
